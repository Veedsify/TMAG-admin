...
import {
    useAdminEbooks, useAdminUpdateEbook,
    useAdminAddVersion, useAdminUpdateVersion, useAdminDeleteVersion,
    useUploadEbookPdf
} from "../../api/hooks";
import type { UpdateEbookRequest, CreateVersionRequest, UpdateVersionRequest, AdminEbookVersion } from "../../api/types";
import {
    ArrowLeft, Plus, Trash2, Upload, LinkIcon, Globe,
    Loader2, FileText, BookOpen, AlertCircle
} from "lucide-react";
import { cn } from "../../lib/utils";
import toast from "react-hot-toast";

type VersionDraft = (CreateVersionRequest | UpdateVersionRequest) & {
    _key: string;
    _id?: number; // existing version id
...
function versionFromExisting(v: AdminEbookVersion): VersionDraft {
    return {
        _key: `v-${++versionKeyCounter}`,
        _id: v.id,
        _isNew: false,
        _fileMode: v.fileUrl ? "link" : "link", // default to link for existing
        _fileName: v.fileUrl?.split("/").pop() ?? undefined,
        label: v.label,
        ...
        fileUrl: v.fileUrl ?? "",
        fileKey: v.fileKey ?? "",
        fileSizeMb: v.fileSizeMb ?? undefined,
        isActive: v.isActive,
...
export default function EbookEditPage() {
    ...
    const { mutateAsync: updateEbook, isPending: isUpdating } = useAdminUpdateEbook();
    const { mutateAsync: addVersion } = useAdminAddVersion();
    const { mutateAsync: updateVersion } = useAdminUpdateVersion();
    const { mutateAsync: deleteVersion } = useAdminDeleteVersion();
    const { mutateAsync: uploadPdf } = useUploadEbookPdf();

    const ebook = ebooks?.find(e => e.id === ebookId);

    const [form, setForm] = useState<Partial<UpdateEbookRequest>>({});
    const [versions, setVersions] = useState<VersionDraft[]>([]);
    const [deletedVersionIds, setDeletedVersionIds] = useState<number[]>([]);
    const [saving, setSaving] = useState(false);
    const [initialized, setInitialized] = useState(false);

    // Initialize form when ebook loads
    useEffect(() => {
        if (ebook && !initialized) {
            setForm({
                ...
                publishedYear: ebook.publishedYear ?? undefined,
                isbn: ebook.isbn ?? "",
                isActive: ebook.isActive,
                isFeatured: ebook.isFeatured,
            });
            setVersions(ebook.versions.map(versionFromExisting));
            setInitialized(true);
        }
    }, [ebook, initialized]);
    ...
    const updateField = (field: keyof UpdateEbookRequest, value: unknown) =>
        setForm(f => ({ ...f, [field]: value }));

    const updateVersionField = (key: string, field: string, value: unknown) =>
        setVersions(prev => prev.map(v => v._key === key ? { ...v, [field]: value, _dirty: true } : v));

    const removeVersion = (key: string) => {
        const version = versions.find(v => v._key === key);
        if (version?._id) {
            setDeletedVersionIds(prev => [...prev, version._id!]);
        }
        setVersions(prev => prev.filter(v => v._key !== key));
    };

    const handleFileUpload = async (key: string, file: File) => {
        updateVersionField(key, "_uploading", true);
        try {
            ...
            updateVersionField(key, "_uploading", false);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Update ebook fields
            await updateEbook({ id: ebookId, data: form as UpdateEbookRequest });

            // Delete removed versions
            for (const vId of deletedVersionIds) {
                await deleteVersion({ ebookId, versionId: vId });
            }
            ...
            for (const v of versions) {
                ...
                } else if (!_isNew && _dirty && _id) {
                    await updateVersion({ ebookId, versionId: _id, data: versionData as UpdateVersionRequest });
                }
            }
    ...
    return (
        <div className="space-y-8 max-w-4xl">
            ...
            <div className="flex items-center gap-4">
                <Link to={`/admin/ebooks/${ebookId}`}
                    className="p-2 text-muted hover:text-heading hover:bg-gray-50 rounded-lg transition-colors">
                    <ArrowLeft className="w-4 h-4" />
                </Link>
                <div>
                    <h1 className="text-2xl font-serif text-heading">Edit Ebook</h1>
                    <p className="text-sm text-muted mt-0.5">{ebook.title}</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <section className="bg-white border border-border-light rounded-2xl overflow-hidden">
                    ...
                    {versions.length === 0 ? (
                        <div className="p-8 text-center">
                            ...
                            <p className="text-sm text-muted">No editions. Add one to enable purchases.</p>
                        </div>
                    ) : (
                        <div className="divide-y divide-border-light">
                            {versions.map((version, idx) => (
                                <VersionEditor
                                    ...
                                    version={version}
                                    index={idx}
                                    onUpdate={(field, value) => updateVersionField(version._key, field, value)}
                ...
                <div className="flex justify-end gap-3">
                    ...
                    <button type="submit" disabled={saving || isUpdating}
        ...
        <div className="p-6 space-y-4">
            <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-heading">
                    {version._isNew && <span className="text-accent text-xs font-normal mr-2">NEW</span>}
                    Edition {index + 1}
            ...
            <div className="grid sm:grid-cols-2 gap-4">
                <FormField label="Label *">
                    <input type="text" value={version.label ?? ""}
            ...
            <div className="grid grid-cols-3 gap-4">
                <FormField label="Price *">
                    <input type="number" value={version.price || ""} step="0.01"
                        onChange={e => onUpdate("price", parseFloat(e.target.value) || 0)}
                        className={INPUT} placeholder="29.99" />
                </FormField>
                <FormField label="Currency *">
                    <input type="text" value={version.currency ?? ""} maxLength={3}
                        onChange={e => onUpdate("currency", e.target.value.toUpperCase())}
                        className={INPUT} placeholder="USD" />
                </FormField>
                <FormField label="Symbol *">
                    <input type="text" value={version.currencySymbol ?? ""}
                        ...
                        {version._uploading ? (
                            ...
                            <div className="flex items-center gap-3 px-4 py-3 bg-green-50 border border-green-200 rounded-xl">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm text-green-800 font-medium truncate">{version._fileName || "PDF attached"}</p>
                                    {version.fileSizeMb && (
