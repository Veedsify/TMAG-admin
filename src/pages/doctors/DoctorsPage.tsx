...
export default function DoctorsPage() {
    ...
    const { data: applications, isLoading: appsLoading } = useDoctorApplications();
    ...
    const handleRevoke = (userId: number, name: string) => {
        if (!confirm(`Are you sure you want to revoke doctor privileges for ${name}?`)) return;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    { label: "Total Doctors", value: stats?.totalDoctors ?? 0, icon: Users },
                    { label: "Pending Apps", value: stats?.pendingApplications ?? 0, icon: ClipboardList },
            ...
            <div className="flex items-center gap-2 border-b border-border-light">
                {([
                    { key: "applications" as Tab, label: "Applications", count: applications?.length ?? 0 },
                    { key: "doctors" as Tab, label: "Verified Doctors", count: doctors?.length ?? 0 },
                ]).map((t) => (
                    <button
                        key={t.key}
                                ...
                                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                                    <div className="flex-1">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 text-sm">
                                            <p className="text-muted"><span className="text-heading">Email:</span> {app.email}</p>
                                            {app.phone && <p className="text-muted"><span className="text-heading">Phone:</span> {app.phone}</p>}
                                            {app.licenseNumber && <p className="text-muted"><span className="text-heading">License:</span> {app.licenseNumber}</p>}
                                            {app.applicationSubmittedAt && <p className="text-muted"><span className="text-heading">Applied:</span> {new Date(app.applicationSubmittedAt).toLocaleDateString()}</p>}
                                        </div>
                                        ...
                                        <div className="flex flex-col gap-2 min-w-[140px]">
                                            {rejectingId === app.userId ? (
                                                ...
                                                <>
                                                    ...
                                                    <button
                                                        onClick={() => setRejectingId(app.userId)}
                            ...
                            >
                                <div className="flex-1">
                                    ...
                                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted">
                                        ...
                                        {doc.licenseNumber && <span>License: {doc.licenseNumber}</span>}
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                                    ...
                                    <button
                                        onClick={() => handleRevoke(doc.userId, `Dr. ${doc.firstName} ${doc.lastName}`)}
